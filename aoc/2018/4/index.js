...
rl.on('close', function() {
  ...
  for (let record of records) {
    ...
    if (shift) {
      ...
      for (let i = fall; i < wu; i++) {
        ...
        if (champ.z < zZz[guard].sleep[i]) {
          champ.z = zZz[guard].sleep[i];
          champ.id = guard;
          champ.m = i;
        }
      }
  ...
  console.log(`id: ${champ.id}, z: ${champ.z}, min: ${champ.m}`);

  console.log(`guard x min: ${champ.id * champ.m}`);
});
